...
The covers are also embedded in the FLAC files as `PICTURE` blocks of type 3
(front cover). Baseline JPEG without an alpha channel is used deliberately —
RGBA PNG is silently ignored by VLC and several other players even when the
metadata block itself is valid.
